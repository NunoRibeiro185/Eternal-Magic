...
static func for_element(element: int) -> ElementStyle:
	...
	match element:
		SpellUtil.Element.Fire: # soft, erratic, bloomy, rising
		...
		SpellUtil.Element.Water: # round, fluid, controlled
		...
		SpellUtil.Element.Ice: # controlled but crisper than water
		...
		SpellUtil.Element.Electric: # sharp, fast, erratic
		...
		SpellUtil.Element.Earth: # rocky: hard chunks, no swirl, no glow, thrown out and falling
		...
		SpellUtil.Element.Poison: # slow, bubbling, controlled
